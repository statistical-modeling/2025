...
order. This means that the model has a variable *"treatmentUngrazed"*,
that has value one for ungrazed plants, and value zero otherwise. So,
fruit yield predicted ($\widehat{Y}$) by this model for grazed plants is:

$$\widehat{Y}_{\text{grazed}} = \beta_0 + \beta_1 \text{diameter} + \beta_2 \times 0$$

While for ungrazed plants we have:

$$\widehat{Y}_{\text{ungrazed}} = \beta_0 + \beta_1 \text{diameter} + \beta_2 \times 1$$
...
the value of the coefficient of the variable *"treatmentUngrazed"*.
Thus, the fitted model estimates that ungrazed plants had on average
...
the linear regression model we have been writting as:
