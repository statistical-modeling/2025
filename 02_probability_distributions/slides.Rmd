...
* The integral of $f(x)$ over the sample space is one.

* This is a mathematical way to say that the area under $f(x)$ is one.
...
* The density function that we have defined is used to express probabilities of random variables, or a  **probability distribution**.

--

* It is a model that attributes probabilities to data, that is, to values of a random variable.

--

* In our example, the random variable is the mean thorax length that
  we got from samples of 30 flies from sites in Australia.
...
* This is one of the most important results in probability theory. We
  have very good reasons to chosse a Normal distribution as our
  statistical model.
...
* TCL tell us that this is good model for any random variable which is
a sum of many other random variables.
* It's a symmetric distribution, always bell-shaped.
...
* The Normal has a **location parameter**, that we'll call $\mu$.
...
* The Normal has a **scale parameter**, that we'll call $\sigma$.
