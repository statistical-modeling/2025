...
corresponds to fitting a separate linear regression for each archipelago
(study site). Thus, there is an independent intercept and slope for the
...
in an assumed distribution. This is a hierarchical model, where slopes
and intercepts for each site follow some probability
distribution, usually Gaussian distributions.
...
We need to define prior distributions for the three parameters of the
...
Assuming the same regression line for every site is greatly
underfitting the data, we need a more complex model.
...
sites. Thus, we can try allowing different intercepts and slopes for
...
same prior for single parameter $\sigma$, the residual standard deviation.
...
adding 34 parameters to the model. We switched from underfitting to possible overfitting. 
...
And then, we can add the regression lines of this model and the of the
...
How variable would these slopes among archipelagoes be, assuming our model
provides a good prediction? We can assess this by simulating the
posterior distributions of the slopes. To achieve this, we first take 
...
reference, we also added the prior distributions of the slopes:
...
The WAIC tells us that the model with partially-polled intercepts and slopes is
about as good at predicting future observations as the model with independent
slopes and intercepts. This suggests that the latter may be mildly overfitted, but our carefully chosen priors for the slopes and intercepts of the model with independent slopes and intercepts have helped to avoid overfitting. One advantage of the model with partially-pooled slopes and intercepts is that this prior choice is done for us, and the effective number of parameters is actually smaller.
...
categorical variable (the site coded as #1), $\beta_0$ is also the slope
for the first level of the categorical variable. And thus
...
site 1. You can get the estimates of intercepts for sites 2 to 18 with:
