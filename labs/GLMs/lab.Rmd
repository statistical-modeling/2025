...
positive values. For such cases, we rely on
...
\begin{align}
Y &\sim Bernoulli(p_i) \\
p_i &= g^{-1}(\alpha + \beta_1 X_{i1} + \beta_2 X_{i2}) \\
g(p_i) &= \alpha + \beta_1 X_{i1} + \beta_2 X_{i2}
\end{align}
$$
...
## Women
m2.pFW <- m2.cf[1] + m2.cf[2] + m2.cf[7]
...
$$
\begin{align}
A_i &\sim Binomial(N_i, \pi_i) \\
logit(\pi_i) &= \beta_{gender_{[i]}} \\
\beta_{gender_{[i]}} &\sim Normal(0, 1.5)
\end{align}
$$
