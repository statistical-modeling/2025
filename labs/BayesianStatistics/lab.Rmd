...
This is rarely a good idea, because the sum of squared residuals always decreases as the model complexity increases. This is because the model has more parameters to fit the data. We can see this by fitting a polynomial model with degree 2 (and beyond).

## Polynomial models

```{r polynomial_model, eval = FALSE}
...
logprob <- sapply( 1:n_samples ,
    ...
    dnorm( d$y , mu , post$sigma[s] , log=TRUE ) # this is the log-likelihood of the data calculated for each posterior sample
    } )
...
# p_waic_i, the effective number of parameters
pWAIC = sum(pWAIC_i)
...
# Strongly regularized priors

This is class of mixture models that attempt to figure out which variables are important for prediction and which are not. The idea is to use a prior that is very skeptical about the importance of the variables, and then let the data tell us which variables are important. This is a form of _automatic variable selection_, and is a very powerful tool. Important variables should get non-zero estimates, while other variables should get estimates close to zero. This is an extreme form of _shrinkage_, where the estimates are pulled towards zero. 

Several methods exist to implement this idea, such as the LASSO, Ridge regression, and Elastic Net. Here, we will use a Bayesian version, called the _horseshoe prior_. For a (potentially large) set of $p$ predictors $\{x_1, x_2, \cdots, x_p\}$, this prior is defined as:

\[
  \begin{aligned}
  y &\sim \text{Normal}(\mu, \sigma) \\
  \mu &= \alpha + \sum_{j=1}^p \beta_j x_j \\
  \alpha &\sim \text{Normal}(0, 1) \\
  \beta_j &\sim \text{Normal}(0, \lambda_j \tau) \\
  \lambda_j &\sim \text{Cauchy}^+(0, 1) \\
  \tau &\sim \text{Cauchy}^+(0, \tau_0) 
  \end{aligned}
\]

The $\lambda_j$ parameters are the _shrinkage parameters_, and they control how much the **individual** $\beta_j$ parameters are pulled towards zero. The $\tau$ parameter is the _global shrinkage parameter_, and it controls how much the **population** of $\beta_j$ parameters are pulled towards zero. The Cauchy distribution is a heavy-tailed distribution, which means that it allows for the possibility of very large values of $\lambda_j$, which in turn allows for the possibility of very large values of $\beta_j$. This is a very flexible prior, and is capable of capturing a wide range of possible relationships between the predictors and the response.

Let's fit a model with the horseshoe prior to the data we simulated before.
Implementing this model directly in rethinking or Stan requires some special techniques to guarantee that the model will properly explore the posterior distribution, see [Piironen & Vehtari (2017)](https://arxiv.org/abs/1707.01694) or [Betancourt (2018)](https://betanalpha.github.io/assets/case_studies/bayes_sparse_regression.html) for details. Here, we use the implementation in the rstanarm package.

```{r horseshoe_prior, eval = FALSE}
library(rstanarm)
library(bayesplot)
library(ggplot2)
m_horseshoe = stan_glm(y ~ x1 + x2 + z1 + z2 + z3, data = d, 
          prior = hs(global_scale = (2/3)/10), 
          prior_intercept = normal(0, 1), 
          chains = 4, cores = 4, iter = 2000)
          
mcmc_intervals(m_horseshoe) +  
  geom_point(data = data.frame(y = c(2:6), x = c(0, 0, 0, -0.4, 0.15)), 
             col =  "red", aes(y = y, x = x)) 
```

The smaller coefficient of x1 is underestimated (but still in the confidence interval), while the larger coefficient of x2 is correct. This is because the horseshoe prior is very skeptical about the importance of the variables, and so it pulls the estimates towards zero. All the other coefficients are correctly estimated as zero.