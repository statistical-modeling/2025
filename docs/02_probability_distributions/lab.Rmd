...
turn right or left to change some aspect of the function. The normal,
or Gaussian, function is always bell-shaped. It has two parameters,
...
continuous variable and the probability of a given measurement.
...
Finally, to get the probability of an interval, you can take the
difference between the accumulated probabilities for the upper limit
and for the lower limit of the interval:
...
=1.86$. This distribution is a statistical model, that is, **a model
that assigns probabilities to measurements**.  It predicts that the
probability of a girl born with length between $40$ and $50$ cm is:
...
under or overestimation of $\mu$ by the sample are equally
likely.
...
means tends to the value of the parametric mean of the sampled
distribution (which in this case is $\mu$).
...
We can check that simulating samples from a normal random variable up to
size n = 5000, with $\mu = 0.2$ and $\sigma = 1$. See how when
...
__Challenge:__ Adapt the previous code to a Poisson distribution with
$\lambda = 0.7$, with a population size of $n = 500$. Then create
different samples with increasing sizes, from 50 to 500 points in 10
intervals.
...
The __Central Limit Theorem__ states that the arithmetic means of
...
Let's check this starting with a uniform random variable in the [0, 100] interval, for example:
...
Since the Central Limit Theorem applies to samples that are large
enough, let's make different tests with increasing sample sizes from
this population.  The function `sample()` executes this sampling from
vector `pop`. First, let's make a single sample of size 5.
...
To repeat this sampling, we will create a loop and save each sample mean in an empty vector.
...
disttributions, a larger sample size may be needed to obtain normal
distribuitions from the sample means than for symmetrical
distributions. Let's compare this effect in the case of Poisson
distributions for different sample sizes and $\lambda$ values.
...
sizes, like 20 or 30. What can you observe about the effect of the
...
Do the same comparison with a more symmetric Poisson random variable
(that is, with a higher Lambda value). The sample size needed to
obtain a normal distribution from the sample means is lower than in
the case of more skewed distributions.
...
backwards. The plot below draws this random walk, starting at position
zero:
