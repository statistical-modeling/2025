...
In [multiple linear regression](../LinearModels/lab2.html), the
...
behind these partial effects is not trivial, because such effects
...
interface](http://www.dagitty.net/dags.html)^[For beginners, we
recommend to start playing with this interface, and then check the
...
# Extra: full luxury models

In some cases, you may want to estimate the full causal model, including all possible paths in the dag that relates X and Y. This is called a "full luxury model".

Let's simulate from the DAG we saw in class and try to fit a complete model:

```{r, eval=FALSE }
dag2 = dagitty('dag { X -> W W -> Y Z -> X Z -> Y A -> Z A -> X B -> Z B -> Y}')
coordinates( dag2 ) <-
    list( x=c(X = 0, W = 1, A = 0, B = 2, Y = 2, Z = 1),
          y=c(X = 0, W = 0, A = -1, B = -1, Y = 0, Z = -1) )
plot(dag2)
...
W = rnorm(N, X)
Y = rnorm(N, W + Z + B)
df = data.frame(X, Y, Z, W, A, B)
```

Remember that the correct adjustment set for estimating the effect of X on Y is {Z, B}.

```{r, eval=FALSE }
lm(Y ~ X + Z + B, data = df) |> coef()
```

This gives the correct estimate, but we can do better and try to fit the full DAG. This is a full luxury model, and it may not be necessary in many cases, but it is a good exercise to understand the implications of the DAG.
...
mFull = ulam(
  alist(
    A ~ normal(aA, sA),
    B ~ normal(aB, sB),
    Z ~ normal(aZ + bAZ * A + bBZ * B, sZ),
    X ~ normal(aX + bAX * A + bZX * Z, sX),
    W ~ normal(aW + bXW * X, sW),
    Y ~ normal(aY + bWY * W + bZY * Z + bBY * B, sY),
    c(bAZ, bAX, bBZ, bZX, bZY, bBY, bXW, bWY) ~ normal(0, 0.3),
    c(aA, aB, aZ, aX, aY, aW) ~ normal(0, 1),
    c(sA, sB, sX, sY, sZ, sW) ~ exponential(1)
  ), data = df, chains = 4, cores = 4, iter = 2000  
)
plot(precis(mFull, prob = 0.95))
abline(v = 1, col = 2)
```

**Challenge**: What if B was unobserved? How would you estimate the effect of X on Y in this case? Think about how we can still use this model even if one of the variables is unobserved.

```{r, eval=FALSE }
# Make B unobserved
df_noB = data.frame(X, Y, W, A, Z)
```

**Extra reading**: The [brms package](https://paulbuerkner.com/brms/) can handle [missing data imputation](https://cran.r-project.org/web/packages/brms/vignettes/brms_missings.html) on the fly, simultaneously fitting the model parameters and estimating the missing values. 